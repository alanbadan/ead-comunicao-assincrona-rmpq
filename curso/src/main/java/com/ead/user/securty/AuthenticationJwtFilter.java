	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		try {
			...
			if(jwtStr != null && jwtProvider.validatejwt(jwtStr)) { // verificando se não é null e se o token esta corretor com o metodo  **public boolean validatejwt(String authToken)** 
				String userId = jwtProvider.getSubjectJwt(jwtStr); // extraindo o userId do token com metodo ** public String getUserIdjwt(String token)**
				String rolesStr = jwtProvider.getClainNameJwt(jwtStr, "roles"); //usando o metodo para extrair as roles 
				UserDetails userDetails = UserDetailsImpl.build(UUID.fromString(userId), rolesStr); // fromString convertendo UUID para String
